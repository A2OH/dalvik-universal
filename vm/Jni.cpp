...
void dvmCallJNIMethod(const dreg_t* args, JValue* pResult, const Method* method, Thread* self) {
    /* On 64-bit, copy args and reconstitute doubles/longs from split dreg_t slots.
     * Wide values (D/J) have low 32 bits in slot[i], high 32 bits in slot[i+1].
     * dvmPlatformInvoke reads each arg as a contiguous uintptr_t, so we must
     * pack the 64-bit value into a single slot for libffi to read correctly. */
    int nArgs = method->registersSize;
    /* Cap VLA to prevent stack overflow on methods with many parameters */
    if (nArgs > 256) {
        ALOGE("dvmCallJNIMethod: nArgs=%d exceeds max 256, aborting", nArgs);
        dvmAbort();
    }
    uintptr_t wideArgs[nArgs > 0 ? nArgs : 1];
    /* First pass: copy all slots */
    for (int i = 0; i < nArgs; i++) wideArgs[i] = args[i];
#if __SIZEOF_POINTER__ > 4
    /* Second pass: reconstitute wide values from split slots */
    {
        const char* shorty = &method->shorty[1]; /* skip return type */
        int idx = (method->accessFlags & ACC_STATIC) ? 0 : 1; /* skip 'this' */
        while (*shorty != '\0') {
            if (*shorty == 'D' || *shorty == 'J') {
                /* Pack: low32 from slot[idx], high32 from slot[idx+1] → one 64-bit value */
                u4 lo = (u4) wideArgs[idx];
                u4 hi = (u4) wideArgs[idx + 1];
                union { u8 ll; uintptr_t parts[1]; } pack;
                pack.ll = ((u8)hi << 32) | (u8)lo;
                wideArgs[idx] = pack.parts[0];
                /* Clear slot[idx+1] to avoid stale data */
                wideArgs[idx + 1] = 0;
                idx += 2;
            } else {
                idx++;
