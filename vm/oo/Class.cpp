    ...
    const u1* pEncodedData, Object* classLoader)
{
     ...
     * this is safe.
     */
    /* assert removed: sizeof(u4) != sizeof(ClassObject*) on 64-bit */
...
bool dvmLinkClass(ClassObject* clazz)
{
        ...
        (strcmp(clazz->descriptor, "Ljava/lang/Class;") == 0))
    {
        ...
        if (gDvm.classJavaLangClass->sfieldCount != CLASS_SFIELD_SLOTS) {
            ALOGW("java.lang.Class has %d static fields (expected %d) — continuing anyway",
                 gDvm.classJavaLangClass->sfieldCount, CLASS_SFIELD_SLOTS);
            /*  Class may have different static fields */
        }
    }
    ...
    if (clazz->status == CLASS_IDX) {
        if (clazz->interfaceCount > 0) {
            ...
            /* Extract each idx individually (pointer-width → u4) */
        ...
        /* assert removed: sizeof(u4) != sizeof(ClassObject*) on 64-bit */
        superclassIdx = (u4)(uintptr_t) clazz->super;
        clazz->super = NULL;
    ...
    const u2* insns)
{
    ...
    if (insns != NULL) {
        ...
        method->nativeFunc = func;  /* fix #43: pointer-width store */
    } else {
...
static int findClassCallback(void* vclazz, void* arg)
{
    ...
    if (strcmp(clazz->descriptor, descriptor) == 0)
        return 1;  /* fix #38: (int)clazz truncates 64-bit pointer */
    return 0;
}
